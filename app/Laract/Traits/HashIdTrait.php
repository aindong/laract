...
use App\Laract\Exceptions\IncorrectIdException;
...
//        'users/{id}/external',
    ...
    protected function decodeHashedIdsBeforeValidation(Array $requestData)
    {
        ...
        if (Config::get('pluggables.hash-id') && isset($this->decode) && !empty($this->decode)) {
    ...
    {
        if (Config::get('pluggables.hash-id')) {
